-- browse_cards: surface occlusion_data so the Create/Browse card editors can
-- render existing occlusion regions without an extra per-card fetch. Adding an
-- OUT column changes the return type, so drop + recreate.
