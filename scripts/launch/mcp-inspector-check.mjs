...
// Explicit production reviewer setup and MCP Inspector CLI check. Uses only its own
...
async function inspect(access,method,tool) {
 const args=['--yes','@modelcontextprotocol/inspector','--cli',resource,'--transport','http','--format','json','--method',method,'--header',`Authorization: Bearer ${access}`];
 if(tool) args.push('--tool-name',tool.name,'--tool-args-json',JSON.stringify(tool.arguments));
 let stdout;
 try { ({stdout}=await runFile('npx',args,{maxBuffer:4*1024*1024,timeout:60000})); }
 catch(e){ if(e.code===5 && e.stdout){ const rejected=JSON.parse(e.stdout); if(rejected.result?.isError) return rejected.result; } throw new Error(`Inspector ${method} ${tool?.name??''} failed (exit ${e.code})`); }
 const result=JSON.parse(stdout);
 return result.result ?? result;
}
async function connect(access){return {listTools:()=>inspect(access,'tools/list'),callTool:tool=>inspect(access,'tools/call',tool),close:async()=>{}};}
...
const deck=await call('create_deck',{name:'MCP Inspector sample'});const deckId=deck.id??deck.project?.id;assert.ok(deckId);
...
await call('list_decks');await call('get_study_queue',{deck_id:deckId});await call('get_card',{card_id:cardId});await call('browse_cards',{deck_id:deckId});await call('rename_deck',{deck_id:deckId,name:'MCP Inspector sample verified'});await call('update_card',{card_id:cardId,back:'Four (4)'});await call('submit_review',{card_id:cardId,grade:'good'});await call('get_deck_stats',{deck_id:deckId});await call('get_study_stats');await call('delete_card',{card_id:cardId});
await client.close();
