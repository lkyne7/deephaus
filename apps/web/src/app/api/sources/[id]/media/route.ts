...
/** Upload an image to embed inline in the editable source document. */
export const POST = withApiTiming(async function POST(
  ...
  if (!(await assertSourceOwner(supabase, sourceId, user!.id))) {
  ...
  const storagePath = `${user!.id}/source-media/${sourceId}/${Date.now()}-${safeFileName(file.name)}`;
