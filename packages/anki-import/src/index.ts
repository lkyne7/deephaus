...
/** Reads one media file's bytes by Anki filename, or null when unavailable. */
export type MediaReader = (filename: string) => Promise<Uint8Array | null>;

export type AnkiImportPhase = "media" | "decks";
...
export interface AnkiImportOptions {
  ...
  /**
   * Lazy media reader for large packages. When omitted, falls back to the
   * in-memory {@link ParsedApkg.media} map (small/eager imports).
   */
  mediaReader?: MediaReader;
  /** Coarse progress reporting (0-100) for durable job tracking. */
  onProgress?: (progress: number, phase: AnkiImportPhase) => void | Promise<void>;
}
...
/** Media upload occupies this slice of the 0-100 progress range. */
const MEDIA_PROGRESS_START = 12;
const MEDIA_PROGRESS_END = 55;
const DECKS_PROGRESS_END = 98;
...
async function uploadMedia(
  ...
  for (const filename of refs) {
    const bytes = options.mediaReader
      ? await options.mediaReader(filename)
    ...
    if (!bytes || !contentType) {
      ...
      const { error } = await supabase.storage.from("card-media").upload(storagePath, bytes, {
        contentType,
        upsert: false,
    ...
    if (options.onProgress && refs.length > 0 && processed % 10 === 0) {
      const frac = processed / refs.length;
      await options.onProgress(
        Math.round(MEDIA_PROGRESS_START + frac * (MEDIA_PROGRESS_END - MEDIA_PROGRESS_START)),
        "media",
      );
    }
  }
...
async function createDeck(
  ...
  importScheduling: boolean,
  onCardsInserted?: (count: number) => void | Promise<void>,
  ...
  for (const batch of chunk(deck.cards, CARD_INSERT_CHUNK)) {
    ...
    await onCardsInserted?.(inserted.length);
...
export async function importAnkiPackage(
  ...
  const totalCards = targetDecks.reduce((sum, deck) => sum + deck.cards.length, 0) || 1;
  let insertedCards = 0;
  ...
  await options.onProgress?.(MEDIA_PROGRESS_END, "decks");
    ...
    const created = await createDeck(
      ...
      async (count) => {
        ...
        if (options.onProgress) {
          const frac = Math.min(1, insertedCards / totalCards);
          await options.onProgress(
            Math.round(MEDIA_PROGRESS_END + frac * (DECKS_PROGRESS_END - MEDIA_PROGRESS_END)),
            "decks",
          );
        }
      },
